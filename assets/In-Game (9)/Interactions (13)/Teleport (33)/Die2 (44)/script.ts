class Die2Behavior extends Sup.Behavior {
  ...
  update() {
    ...
    if (Math.abs(Game.playerBehavior.position.x - this.position.x) <= this.size.x / 2 && Math.abs(Game.playerBehavior.position.y - this.position.y) <= this.size.y / 2) {
      this.triggered = true;
      Fade.start(Fade.Direction.Out, { duration: 100 }, () => { Sup.loadScene("In-Game/Maps/Niveau/NIV2") });
    }
  }

}
Sup.registerBehavior(Die2Behavior);
